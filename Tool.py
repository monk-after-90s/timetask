...
class ExcelTool(object):
    ...
    def create_excel(self, file_name: str = __file_name, sheet_name=__sheet_name,
                     history_sheet_name=__history_sheet_name):
        # 文件路径
    ...
    # 将历史任务迁移指历史Sheet
    def moveTasksToHistoryExcel(self, tasks, file_name=__file_name, sheet_name=__sheet_name,
                                history_sheet_name=__history_sheet_name):
        # 文件路径
        workbook_file_path = self.get_file_path(file_name)
        ...
        if os.path.exists(workbook_file_path):
            ...
            print(
                f"将任务Sheet({sheet_name})中的 过期任务 迁移指 -> 历史Sheet({history_sheet_name}) 完毕~ \n 迁移的任务ID为：{hisIds}")
    ...
    # 写入数据
    def write_columnValue_withTaskId_toExcel(self, taskId, column: int, columnValue: str, file_name=__file_name,
                                             sheet_name=__sheet_name):
        # 读取数据
...
class TimeTaskModel:
    ...
    def get_cicleDay(self, circleStr):
        ...
        # 每周X
                           ...
                           "每星期一", "每星期二", "每星期三", "每星期四", "每星期五", "每星期六", "每星期日",
                           "每星期天"]:
            # 每天、每周X等
            g_circle = circleStr
    ...
    def get_time(self, timeStr):
        ...
        if time_good1:
            ...
            digits = {'零': 0, '一': 1, '二': 2, '三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9,
                      ...
                      '二十八': 28, '二十九': 29, '三十': 30,
                      '三十一': 31, '三十二': 32, '三十三': 33, '三十四': 34, '三十五': 35, '三十六': 36, '三十七': 37,
                      '三十八': 38, '三十九': 39, '四十': 40,
                      '四十一': 41, '四十二': 42, '四十三': 43, '四十四': 44, '四十五': 45, '四十六': 46, '四十七': 47,
                      '四十八': 48, '四十九': 49, '五十': 50,
                      '五十一': 51, '五十二': 52, '五十三': 53, '五十四': 54, '五十五': 55, '五十六': 56, '五十七': 57,
                      '五十八': 58, '五十九': 59, '六十': 60, '半': 30}
    ...
    def get_gropID_withGroupTitle(self, groupTitle, channel_name):
        ...
        if channel_name == "wx":
            ...
            tempRoomId = ""
            try:
                contacts_list: dict = self._last_msg.client.fetch_contacts_list(self._last_msg.app_id)
                for chatroom in contacts_list['data']['chatrooms']:
                    chatroom_info = self._last_msg.client.get_chatroom_info(self._last_msg.app_id, chatroom)
